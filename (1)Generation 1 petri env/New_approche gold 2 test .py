...
class Petri_env:
    ...
    def firing (self,Transition):
        ...
        if all([Next_state[i] >= 0 for i in range(self.NPLACES)]):
            ...
            if Next_state[-1]> self.delivered :
               
                delivery=True    
                self.delivered=Next_state[-1]  
            
            return (Next_state,delivery)
        
        else : 
            print ("firing Halted ")
            return (current_state,delivery)
...
class DynaQ :
    ...
    def __init__(self,petri_env,exp_rate=0.1, lr=0.3, n_steps=1, episodes=1):
        ...
        max_token = max(self.state)

        self.state_space =list(itertools.product((range(0,5)), repeat = petri_env.NPLACES))
    ...
    def Train(self):       
        ...
        for ep in range(self.episodes):  
            ...
            while not self.terminal:
                ...
                Next_state,delivery =self.env.firing(action)
                reward=self.Reward(self.env.firing(action))      
...
if __name__ == "__main__":
    N_EPISODES =1
    # comparison
    agent = DynaQ(env,n_steps=0, episodes=N_EPISODES)
    agent.Train()
    steps= agent.steps_per_episode
0
#%%
print (agent.state_space)
